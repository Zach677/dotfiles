...
# [ -f "$HOME/.local/etc/private.zsh" ] && source "$HOME/.local/etc/private.zsh"
...
path=(
    /opt/homebrew/bin
    /opt/homebrew/sbin
    # Homebrew 安装的但未默认链接的软件 (keg-only)
    "/opt/homebrew/opt/openjdk/bin"
    "/opt/homebrew/opt/curl/bin"
    "/opt/homebrew/opt/llvm/bin"
    "/opt/homebrew/opt/qt@5/bin"
    "/opt/homebrew/opt/mongodb-community/bin" # 使用 opt 路径而非 Cellar 路径
    $path
)
...
ghdep() {
    gh search prs --owner innei is:open author:app/renovate archived:false --json url --jq ".[].url" | xargs -I URL bash -c 'echo "Approving & merging: URL" && gh pr review --approve URL && gh pr merge --squash --auto URL'
}
...
#proxy
# export export https_proxy=http://127.0.0.1:6152;export http_proxy=http://127.0.0.1:6152;export all_proxy=socks5://127.0.0.1:6153
